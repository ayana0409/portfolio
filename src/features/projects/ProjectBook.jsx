...
export default function ProjectBook() {
  ...
  return (
    <section
      id="projects"
      className="relative h-screen h-[100dvh] w-full pt-20 pb-4 px-4 sm:px-8 lg:px-12 flex flex-col justify-between overflow-hidden snap-start snap-always bg-slate-950 text-white select-none"
    >
      {/* ─── Ambient Glow in Background ──────────────────────────────────────── */}
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[700px] h-[350px] bg-blue-600/10 blur-[160px] rounded-full pointer-events-none" />
      ...
      <div className="relative z-10 max-w-6xl mx-auto w-full flex items-center justify-between gap-4 pb-2 border-b border-slate-800/80">
        <div className="flex items-center gap-3">
          <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-mono tracking-widest uppercase bg-slate-900/80 text-blue-400 border border-slate-800">
            <span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse" />
            <span>04 / 3D BOOK</span>
          </div>
          <h2 className="text-xl sm:text-2xl font-black uppercase tracking-[0.12em] text-white">
        ...
        <div className="flex items-center gap-3 text-xs font-mono">
          <span className="text-slate-400">
            PAGE <span className="text-white font-bold text-sm">0{currentIndex + 1}</span> / 0{totalProjects}
          </span>
      ...
      {/* ─── 3D Book Stage (Fitted Perfectly Inside 100vh) ───────────────────── */}
      <div className="relative z-10 flex-1 min-h-0 max-w-6xl mx-auto w-full flex items-center justify-center my-auto py-2 [perspective:2200px]">
        {/* Book Outer Frame */}
        <div className="relative w-full h-[calc(100vh-190px)] max-h-[520px] min-h-[380px] rounded-2xl bg-slate-950 border border-slate-800 shadow-2xl p-2 [transform-style:preserve-3d]">
          {/* Left Spine Binding (Realistic Book Hinge) */}
          <div className="absolute inset-y-0 left-0 w-8 md:w-10 bg-gradient-to-r from-slate-950 via-slate-900 to-transparent border-r border-slate-800/80 rounded-l-2xl z-40 pointer-events-none flex flex-col justify-between items-center py-6">
            <div className="w-1 h-6 bg-blue-500/40 rounded-full" />
            <span className="[writing-mode:vertical-rl] rotate-180 text-[8px] font-mono tracking-widest text-slate-500 uppercase">
              PORTFOLIO BINDING
            </span>
            <div className="w-1 h-6 bg-blue-500/40 rounded-full" />
          </div>
          ...
          {projects.map((project, idx) => {
            ...
            return (
              <div
                ...
                className={`absolute inset-2 pl-8 md:pl-14 pr-4 sm:pr-8 py-5
              ...
              >
                ...
                <div
                  ...
                  className="absolute inset-y-0 left-0 w-32 bg-gradient-to-r from-black/95 via-black/40 to-transparent pointer-events-none z-30 opacity-0"
                />
                ...
                <div className="flex items-center justify-between pb-3 border-b border-slate-800/80">
                  ...
                  <span className="text-[11px] font-mono text-blue-400 uppercase tracking-widest hidden sm:inline-block">
                    Page 0{pageNumber} of 0{totalProjects}
                  </span>
                </div>

                {/* Page Body (2/3 Info & Description, 1/3 Image Preview) */}
                <div className="grid md:grid-cols-12 gap-6 my-auto py-2 items-center w-full">
                  {/* Left Column (2/3): Info, Description, Tech Stack & CTAs */}
                  <div className="md:col-span-8 space-y-3.5 text-left pr-0 md:pr-4">
                    <h3 className="text-xl sm:text-2xl lg:text-3xl font-black uppercase tracking-tight text-white leading-snug">
                      {title}
                    </h3>
                    ...
                    <div className="space-y-1 pt-1">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {project.technologies?.map((tech) => (
                          <span
                            key={tech}
                            className="px-2.5 py-0.5 text-xs font-mono rounded-md bg-slate-950 border border-slate-800 text-slate-300 hover:border-blue-500/50 hover:text-white transition-colors"
                          >
                            {tech}
                          </span>
                        ))}
                    ...
                    <div className="pt-2 flex flex-wrap items-center gap-3">
                      <button
                        ...
                        className="group relative inline-flex items-center gap-2 px-5 py-2.5 rounded-xl
                          bg-blue-600 hover:bg-blue-500 text-white font-semibold text-xs
                          uppercase tracking-[0.18em] shadow-md shadow-blue-600/30 hover:shadow-blue-500/50
                      ...
                      {project.links?.source && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-xs font-mono tracking-widest uppercase
                  ...
                  >
                    <div className="relative aspect-[4/3] sm:aspect-[1/1] max-h-[260px] w-full rounded-2xl bg-gradient-to-tr from-slate-950 via-slate-900 to-blue-950 border border-slate-800 group-hover:border-blue-500/60 overflow-hidden shadow-xl group-hover:shadow-[0_0_30px_rgba(59,130,246,0.35)] transition-all duration-300 transform group-hover:scale-[1.02] flex flex-col justify-between p-3.5">
                ...
                <div className="pt-2 border-t border-slate-800/60 flex items-center justify-between text-[11px] font-mono text-slate-500">
                  <span>{t('about.name', 'Dương Đoàn Thuận')} / Backend Portfolio</span>
      ...
      <div className="relative z-10 max-w-6xl mx-auto w-full flex items-center justify-between pt-2 pb-1 border-t border-slate-800/80">
        <div className="text-[11px] font-mono text-slate-400 hidden sm:block">
          USE <kbd className="px-1 py-0.5 rounded bg-slate-900 border border-slate-800 text-white">←</kbd> / <kbd className="px-1 py-0.5 rounded bg-slate-900 border border-slate-800 text-white">→</kbd> TO TURN
        </div>
        ...
        <div className="flex items-center gap-2.5 mx-auto sm:mx-0">
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-1.5 rounded-full
              ...
              text-white text-[11px] font-mono tracking-wider uppercase transition-all active:scale-95 disabled:opacity-40 cursor-pointer"
          >
            <span>← PREV</span>
          </button>
