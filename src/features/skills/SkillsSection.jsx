...
export default function SkillsSection() {
  ...
  return (
    <section
      id="skills"
      ref={containerRef}
      className="relative h-screen h-[100dvh] w-full overflow-hidden flex flex-col justify-between items-center pt-20 pb-4 px-4 sm:px-8 lg:px-12 max-w-5xl mx-auto snap-start snap-always select-none"
    >
      ...
      <div className="text-center space-y-1 mb-2 relative z-10 w-full">
        ...
        <p className="text-[11px] sm:text-xs font-mono tracking-wider uppercase text-slate-400 max-w-lg mx-auto leading-relaxed">
          {currentLang === 'vi'
            ? 'RÊ CHUỘT VÀO TỪNG HÀNG ĐỂ MỞ RỘNG VÀ XEM CHI TIẾT CÔNG NGHỆ'
            : 'HOVER OVER EACH CATEGORY TO EXPAND DETAILED TECH STACK'}
        </p>
      </div>

      {/* ─── Vertical Stack Container (Accordion Flow) ─────────────────────── */}
      <div className="w-full space-y-2.5 my-auto relative z-10">
        {categories.map((cat) => {
          ...
          return (
            <div
              ...
              className={`skill-stack-row group relative rounded-2xl transition-all duration-300 cursor-pointer overflow-hidden ${
                isActive
                  ? 'bg-slate-900/90 border border-blue-500/60 shadow-[0_0_30px_rgba(59,130,246,0.18)] p-4 sm:p-5'
                  : 'bg-slate-900/40 border border-slate-800/80 hover:border-slate-700 hover:bg-slate-900/60 p-3 sm:p-3.5 opacity-75 hover:opacity-100'
              }`}
            >
              {/* Row Header Bar */}
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 sm:gap-4">
                  ...
                  <span
                    className={`font-mono text-xs font-bold px-2.5 py-0.5 rounded-md transition-colors ${
                      isActive
                  ...
                  {/* Category Title */}
                  <h3
                    className={`font-bold tracking-wide uppercase transition-colors text-sm sm:text-base ${
                      isActive
                        ? 'text-white'
              ...
              <div
                className={`transition-all duration-300 ease-out overflow-hidden ${
                  isActive
                    ? 'max-h-36 opacity-100 mt-3 pt-3 border-t border-slate-800/80'
              ...
              >
                <div className="flex flex-wrap gap-2 pt-0.5">
                  {cat.items.map((skill) => (
                    <span
                      key={skill}
                      className="inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-mono font-medium
